...
def find_face_neighbors(mesh: Trimesh, num_hops: int) -> List[Set[int]]:
...
def find_vertex_neighbors(mesh: Trimesh, num_hops: int) -> List[Set[int]]:
...
class IcoSphereRef:
    ...
    def get_neighbor_mapping(self, rank: int, num_hops: int) -> List[Set[int]]:
        if (rank, num_hops) not in self.neighbor_maps.keys():
            ico = self.get_icosphere(rank=rank, refine=True)
            print(f"Building neighbor mapping {rank}-{num_hops}")
            if self.node_type == "face":
                self.neighbor_maps[(rank, num_hops)] = find_face_neighbors(ico, num_hops=num_hops)
            elif self.node_type == "vertex":
                self.neighbor_maps[(rank, num_hops)] = find_vertex_neighbors(ico, num_hops=num_hops)
            else:
                raise NotImplementedError(f"Unsupported node type {self.node_type}")
            print(f"Building neighbor mapping Rank:{rank} Hops:{num_hops} -- DONE")
        return self.neighbor_maps[(rank, num_hops)]
